...
@Inline
inline fun ComposeScope.Stack(
    controller: Controller = Controller,
    onReceive: FrameLayout.() -> Unit = {},
    onLayout: StackScope.() -> Unit
) {
    with(FrameLayout(Store.activity)) {
        controller.controlAttributes(Container, this)

        addChild(this)
        onReceive()

        val scope = StackScope(this)
        scope.onLayout()
    }
}

open class StackScope(Container: FrameLayout) : ComposeScope(Container) {
