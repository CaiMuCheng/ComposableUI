...
@Inline
inline fun ComposeScope.VerticalSlider(
    controller: Controller = Controller,
    onReceive: ScrollView.() -> Unit = {},
    onLayout: VerticalSliderContainerScope.() -> Unit
) {
    val view = ScrollView(Store.activity)
    with(view) {
        ...
        val scope = VerticalSliderContainerScope(this)
        scope.onLayout()
    }
}

@Inline
inline fun ComposeScope.HorizontalSlider(
    controller: Controller = Controller,
    onReceive: HorizontalScrollView.() -> Unit = {},
    onLayout: HorizontalSliderContainerScope.() -> Unit
) {
    val view = HorizontalScrollView(Store.activity)
    with(view) {
        controller.controlAttributes(Container, this)
        ...
        val scope = HorizontalSliderContainerScope(this)
        scope.onLayout()
    }
}

open class VerticalSliderContainerScope(Container: ScrollView) : ComposeScope(Container) {

}

open class HorizontalSliderContainerScope(Container: HorizontalScrollView) :
    ComposeScope(Container) {
