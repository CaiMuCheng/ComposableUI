...
inline fun ComposeScope.FloatingActionButton(
    crossinline onClick: () -> Unit,
    icon: AbstractPainter,
    elevation: Float = 8.dp,
    controller: Controller = Controller,
    onReceive: () -> Unit = {}
) {
    with(com.google.android.material.floatingactionbutton.FloatingActionButton(Store.activity)) {
        controller.controlAttributes(Container, this)
        setOnClickListener {
            onClick()
        }

        compatElevation = elevation
        setImageBitmap(icon.getImageBitmap())
