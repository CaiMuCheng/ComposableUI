...
import com.mucheng.compose.map.AbstractPainter
import com.mucheng.compose.map.EmptyPainter
import com.mucheng.compose.map.ResourcePainter
import com.mucheng.compose.scope.ComposeScope

inline fun ComposeScope.Image(
    painter: AbstractPainter,
    scaleType: ImageView.ScaleType?=null,
    controller: Controller = Controller,
    onReceive: ImageView.() -> Unit = {}
) {
    with(ImageView(Store.activity)) {
        controller.controlAttributes(Container, this)
        setImageBitmap(painter.getImageBitmap())
        if(scaleType != null) {
...
fun ComposeScope.painterResource(resource: Int): ResourcePainter {
    return ResourcePainter(Container.context, resource)
}

fun ComposeScope.emptyPainter(): EmptyPainter {
    return EmptyPainter(Container.context)
}